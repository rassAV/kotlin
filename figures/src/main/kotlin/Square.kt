class Square(x: Float, y: Float, private var edge: Float) : Figure(x, y), Movable, Transforming {
    fun getEdge(): Float = edge

    override fun area(): Float = edge * edge

    override fun move(dx: Float, dy: Float) { x += dx; y += dy }

    override fun resize(zoom: Float) { edge *= zoom }
